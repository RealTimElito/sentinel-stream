...
def map_cic_ids2017_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    Args:
        df: Raw CIC-IDS2017 dataframe
        
    Returns:
        Mapped dataframe with standard column names
    """
    # Common CIC-IDS2017 column name variations
    column_mapping = {}
    
    # Source IP - try multiple variations
    for col in ['Source IP', 'Src IP', 'SourceIP', 'src_ip', 'Source IP', 'SourceIP']:
        if col in df.columns:
            column_mapping[col] = 'src_ip'
            break
    
    # Destination IP - try multiple variations
    for col in ['Destination IP', 'Dst IP', 'DestinationIP', 'dst_ip', 'Destination IP', 'DestinationIP']:
        if col in df.columns:
            column_mapping[col] = 'dst_ip'
            break
    
    # Source Port - handle spaces
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Source Port', 'Src Port', 'SourcePort', 'src_port'] or 'Source Port' in col_clean:
            column_mapping[col] = 'src_port'
            break
    
    # Destination Port - handle spaces
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Destination Port', 'Dst Port', 'DestinationPort', 'dst_port'] or 'Destination Port' in col_clean:
            column_mapping[col] = 'dst_port'
            break
    
    # Protocol - handle spaces
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Protocol', 'protocol']:
            column_mapping[col] = 'protocol'
            break
    
    # Timestamp - handle spaces
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Timestamp', 'Flow Duration', 'FlowDuration', 'timestamp'] or 'Flow Duration' in col_clean:
            column_mapping[col] = 'timestamp'
            break
    
    # Packet size (use Total Length or similar) - handle spaces
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Total Length of Fwd Packets', 'Total Fwd Packets', 
                         'Total Length', 'packet_size', 'Packet Length'] or 'Total Length of Fwd Packets' in col_clean:
    ...
    for col in df.columns:
        col_clean = col.strip()
        if col_clean in ['Label', 'label', 'Attack', 'attack']:
    ...
    # Create standard dataframe - preserve all original columns if requested
    if preserve_all_features:
        # Start with a copy of the original dataframe (with renamed columns)
        result = df_mapped.copy()
        # Remove columns we'll recreate to avoid duplicates
        cols_to_remove = ['src_ip', 'dst_ip', 'src_port', 'dst_port', 'protocol', 'timestamp', 'packet_size', 'label']
    ...
    if 'src_port' in df_mapped.columns:
        result['src_port'] = pd.to_numeric(df_mapped['src_port'], errors='coerce').fillna(0).astype(int)
    else:
        result['src_port'] = pd.Series([0] * num_rows, index=df.index, dtype=int)
    
    if 'dst_port' in df_mapped.columns:
        result['dst_port'] = pd.to_numeric(df_mapped['dst_port'], errors='coerce').fillna(0).astype(int)
    else:
    ...
    if 'protocol' in df_mapped.columns:
        ...
        if protocol.dtype == 'object' or (len(protocol) > 0 and isinstance(protocol.iloc[0], str)):
            ...
            result['protocol'] = protocol.astype(str).str.upper().map(protocol_map).fillna(6).astype(int)
        else:
        ...
        if timestamp.dtype == 'object':
            try:
                ...
                if timestamp_parsed.notna().any():
                    result['timestamp'] = (timestamp_parsed - timestamp_parsed.min()).dt.total_seconds().fillna(0)
                else:
    ...
    if 'packet_size' in df_mapped.columns:
        result['packet_size'] = pd.to_numeric(df_mapped['packet_size'], errors='coerce').fillna(0).astype(int)
    else:
...
def load_cic_ids2017(data_path: str, max_files: Optional[int] = None, auto_download: bool = False) -> pd.DataFrame:
    """
    ...
    if not csv_files:
        ...
        if not csv_files:
            ...
            logger.info("   Or use: python scripts/prepare_dataset.py --dataset CIC-IDS2017 --download")
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'src_ip' in df.columns and 'dst_ip' in df.columns:
        ...
        if all_zero_ips:
            ...
            logger.info("All IPs are default values. Using flow-based identifiers (preserving raw data)...")
            
            # Create unique flow identifiers from actual data
            if 'src_port' in df.columns and 'dst_port' in df.columns:
                # Use port combinations as flow identifiers - these represent actual endpoints
                # Each unique port combo becomes a "node" in our graph
                flow_id = (df['src_port'].astype(str) + '_' + 
                          df['dst_port'].astype(str) + '_' + 
                          df.get('protocol', pd.Series([6] * len(df))).astype(str))
                
                # Create node identifiers from ports (each port becomes a node)
                # Source port -> source node, destination port -> destination node
                unique_src_ports = df['src_port'].unique()
                unique_dst_ports = df['dst_port'].unique()
                
                # Map ports to node IDs (using port numbers as identifiers)
                # This preserves the actual port information from the dataset
                df['src_ip'] = 'port_' + df['src_port'].astype(str)
                df['dst_ip'] = 'port_' + df['dst_port'].astype(str)
                
                logger.info(f"Using {len(unique_src_ports)} unique source ports and {len(unique_dst_ports)} unique destination ports as nodes")
            ...
            if removed > 0:
                logger.info(f"Removed {removed} rows with invalid IP addresses ({removed/initial_len*100:.1f}%)")
    else:
        # If no IP columns, use flow-based identifiers from actual data
        logger.info("No IP address columns found. Using flow-based identifiers (preserving raw data)...")
    ...
    # Remove duplicates
    initial_len = len(df)
    df = df.drop_duplicates(subset=['src_ip', 'dst_ip', 'src_port', 'dst_port', 'timestamp'], keep='first')
    removed = initial_len - len(df)
    if removed > 0:
...
def main():
    ...
    parser.add_argument(
        '--download',
        action='store_true',
        help='Attempt to download dataset if not found'
    )
    parser.add_argument(
        '--force-download',
        action='store_true',
